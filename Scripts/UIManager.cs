...
public class UIManager : MonoBehaviour
{
    ...
    [SerializeField]
    private RectTransform mainMenuButton;
    ...
    public IEnumerator ShowEndScreen()
    {
        ...
        yield return ScaleEndButons();
    }

    public IEnumerator ScaleEndButons()
    {
        playAgainButton.gameObject.SetActive(true);
        playAgainButton.localScale = Vector3.zero;
        playAgainButton.LeanScale(Vector3.one, 0.2f);
        mainMenuButton.gameObject.SetActive(true);
        mainMenuButton.localScale = Vector3.zero;
        mainMenuButton.LeanScale(Vector3.one, 0.2f);
        yield return new WaitForSeconds(0.2f);
    }
    ...
    public IEnumerator HideEndScreen()
    {
        ...
        StartCoroutine(ScaleDown(mainMenuButton));

        yield return new WaitForSeconds(0.5f);
        yield return HideOverlay();
    }

    public IEnumerator SetUndoRedoInactive(float delay)
    {
        bool undoBuffer = undoButton.interactable;
        bool redoBuffer = redoButton.interactable;

        SetUndoInteractable(false);
        SetRedoInteractable(false);
        yield return new WaitForSeconds(delay);
        SetUndoInteractable(undoBuffer);
        SetRedoInteractable(redoBuffer);
    }

    public void SetUndoInteractable(bool set)
    {
        undoButton.interactable = set;
    }
    public void SetRedoInteractable(bool set)
    {
        redoButton.interactable = set;
    }

    public void ToggleShowUndoRedo()
    {
        undoButton.gameObject.SetActive(!undoButton.gameObject.activeSelf);
        redoButton.gameObject.SetActive(!redoButton.gameObject.activeSelf);
    }

    public void OnMainMenuClicked()
    {
        gameManager.SaveGame();
        Debug.Log(PlayerPrefs.GetString("GameSave"));
